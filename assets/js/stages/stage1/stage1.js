...
import {RegisterTemplateTool} from "../../template/classes/tools/RegisterTemplateTool";



let splittedPath = window.location.pathname.split('/');
let templateAction = splittedPath[splittedPath.length -1] ;
let stage = parseInt(splittedPath[splittedPath.length -2]) ;
if(stage !== 1) throw new Error('invalid value for stage') ;

let templateInterface = new TemplateInterface( stage , templateAction  ) ;

if(templateAction === 'create') templateInterface .createTemplate() ;
else if (templateAction === 'load') templateInterface .loadTemplate();
...
let toolsList = [
    toolBox. addTool(new RegisterTemplateTool(templateInterface)),
